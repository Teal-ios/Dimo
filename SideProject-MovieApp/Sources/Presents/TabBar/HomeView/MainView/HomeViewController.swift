...
//        setNavigation()
    ...
    override func setupBinding() {
        let input = HomeViewModel.Input(categoryButtonTapped: self.homeView.categoryButton.rx.tap.withLatestFrom(self.categoryTitle).asSignal(onErrorJustReturn: "영화"),
                                        heroPlusButtonTapped: self.homeView.mbtiHeroMoreButton.rx.tap,
                                        characterPlusButtonTapped: self.homeView.characterMoreButton.rx.tap,
                                        mbtiRecommendPlusButtonTapped: self.homeView.recommendMoreButton.rx.tap,
                                        hotMoviePlusButtonTapped: self.homeView.nowHotMoreButton.rx.tap,
                                        posterCellSelected: posterCellSelected,
                                        mbtiHeroCellSelected: self.mbtiHeroCellSelected,
                                        mbtiRecommendCellSeleted: self.mbtiRecommendCellSeleted,
                                        hotMovieCellSelected: self.hotMovieCellSelected,
                                        viewDidLoad: self.viewDidLoadTrigger,
                                        okAlertButtonTapped: self.homeView.alertView.okButton.rx.tap,
                                        cancelAlertButtonTapped: self.homeView.alertView.cancelButton.rx.tap)
...
extension HomeViewController {
    func setMbtiHeroDataSource() {
        ...
        let myMomentumHeader = UICollectionView.SupplementaryRegistration<MyMomentumHeaderView>(elementKind: MyMomentumHeaderView.identifier) { [weak self] supplementaryView, elementKind, indexPath in
            ...
            self.viewModel.mbti
                .bind { mbti in
                    supplementaryView.titleLabel.text = "\(mbti)가 주인공인 영화"
                }
                .disposed(by: disposeBag)
        }
...
extension HomeViewController {
    func setCharacterDataSource() {
        ...
        let myMomentumHeader = UICollectionView.SupplementaryRegistration<MyMomentumHeaderView>(elementKind: MyMomentumHeaderView.identifier) { [weak self] supplementaryView, elementKind, indexPath in
            ...
            self.viewModel.mbti
                .bind { mbti in
                    supplementaryView.titleLabel.text = "스포주의! \(mbti) 캐릭터 모아보기"
                }
                .disposed(by: disposeBag)
        }
...
extension HomeViewController {
    func setRecommendDataSource() {
        ...
        let myMomentumHeader = UICollectionView.SupplementaryRegistration<MyMomentumHeaderView>(elementKind: MyMomentumHeaderView.identifier) { [weak self] supplementaryView, elementKind, indexPath in
            ...
            self.viewModel.mbti
                .bind { mbti in
                    supplementaryView.titleLabel.text = "\(mbti)가 관심있는 영화/애니"
                }
                .disposed(by: disposeBag)
        }
